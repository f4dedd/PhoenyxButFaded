...
namespace Pheonyx.Maps
{
	public struct Leaderboard
	{
		...
		public Leaderboard(string mapID, byte[] buffer)
		{
			...
			Path ??= $"{Phoenyx.Constants.USER_FOLDER}/pbs/{MapID}";
		...
		public struct Score
		{
			...
			public Score(byte[] buffer)
			{
				...
				foreach (var entry in (Godot.Collections.Dictionary<string, bool>)Json.ParseString(FileBuffer.GetString((int)FileBuffer.GetUInt32())))
				{
					Modifiers[entry.Key] = entry.Value;
				}
			}
