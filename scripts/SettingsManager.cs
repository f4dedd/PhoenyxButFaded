...
namespace Pheonyx
{
	public partial class SettingsManager : Control
	{
		...
		public override void _Ready()
		{
			...
			foreach (var holder in Holder.GetNode("Sidebar").GetNode("Container").GetChildren())
			{
			...
			var profiles = Holder.GetNode("Header").GetNode<OptionButton>("Profiles");
			var profileEdit = Holder.GetNode("Header").GetNode<LineEdit>("ProfileEdit");
			var skins = Holder.GetNode("Categories").GetNode("Visuals").GetNode("Container").GetNode("Skin").GetNode<OptionButton>("OptionsButton");
			var spaces = Holder.GetNode("Categories").GetNode("Visuals").GetNode("Container").GetNode("Space").GetNode<OptionButton>("OptionsButton");
			...
			profileEdit.TextSubmitted += (string text) =>
			{
				...
				if (File.Exists($"{Phoenyx.Constants.USER_FOLDER}/profiles/{text}.json"))
				{
					ToastNotification.Notify($"Profile {text} already exists!");
					return;
				}

				File.WriteAllText($"{Phoenyx.Constants.USER_FOLDER}/profiles/{text}.json", File.ReadAllText($"{Phoenyx.Constants.USER_FOLDER}/profiles/default.json"));
				UpdateSettings();
			};
			...
			profiles.ItemSelected += (long item) =>
			{
				...
				File.WriteAllText($"{Phoenyx.Constants.USER_FOLDER}/current_profile.txt", profile);
			...
			{
				OS.ShellOpen($"{Phoenyx.Constants.USER_FOLDER}/skins/{Phoenyx.Settings.Skin}");
			};
			Holder.GetNode("Categories").GetNode("Other").GetNode("Container").GetNode("RhythiaImport").GetNode<Button>("Button").Pressed += () =>
			{
				...
				var file = Godot.FileAccess.Open($"{OS.GetDataDir()}/SoundSpacePlus/settings.json", Godot.FileAccess.ModeFlags.Read);
		...
		public static void ShowSettings(bool show = true)
		{
			...
			var tween = Settings.CreateTween();
		...
		public static void UpdateSettings(bool connections = false)
		{
			...
			var skins = Holder.GetNode("Categories").GetNode("Visuals").GetNode("Container").GetNode("Skin").GetNode<OptionButton>("OptionsButton");
			var profiles = Holder.GetNode("Header").GetNode<OptionButton>("Profiles");
			string currentProfile = File.ReadAllText($"{Phoenyx.Constants.USER_FOLDER}/current_profile.txt");
			...
			foreach (string path in Directory.GetDirectories($"{Phoenyx.Constants.USER_FOLDER}/skins"))
			{
			...
			foreach (string path in Directory.GetFiles($"{Phoenyx.Constants.USER_FOLDER}/profiles"))
			{
			...
			foreach (ScrollContainer category in Holder.GetNode("Categories").GetChildren())
			{
				foreach (Panel option in category.GetNode("Container").GetChildren())
				{
					...
					{
						var slider = option.GetNode<HSlider>("HSlider");
						var lineEdit = option.GetNode<LineEdit>("LineEdit");
					...
					{
						var checkButton = option.GetNode<CheckButton>("CheckButton");
